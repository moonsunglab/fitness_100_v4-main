...
def predict_physical_age(req: PhysicalAgeRequest):
    ...
    if idx_from_low == n_grades: idx_from_low -= 1
    ...
    assessment_id = None
    if req.user_id:
        # [수정] measured_at 필드 추가 (현재 UTC 시간)
        row = {
            "user_id": req.user_id, "sex": req.sex,
            "sit_ups": req.sit_ups, "flexibility": req.flexibility, "jump_power": req.jump_power, "cardio_endurance": req.cardio_endurance,
            "lo_age_value": lo_age_value, "lo_age_tier_label": lo_age_tier_label, "tier_index": grade_idx,
            "percentile": avg_q * 100.0, "weak_point": weak_point, "detail_quantiles": q_dict,
            "measured_at": datetime.now(timezone.utc).isoformat() # [중요] 날짜 저장
        }
        try:
            r = requests.post(_sb_table_url("physical_age_assessments"), headers=_sb_json_headers(True), json=row)
            if r.status_code < 400: 
                data = r.json()
                if data and isinstance(data, list): assessment_id = data[0].get("id")
                elif data: assessment_id = data.get("id")
        except Exception as e:
            print(f"[WARN] DB 저장 실패: {e}")

    return PhysicalAgeResponse(
        lo_age_value=lo_age_value, lo_age_tier_label=lo_age_tier_label, percentile=avg_q*100, weak_point=weak_point, tier_index=grade_idx, 
        detail_quantiles=q_dict, avg_quantile=avg_q, grade_index=grade_idx, grade_label=lo_age_tier_label, assessment_id=assessment_id
    )
